(function($){
	...
	var methods = {
		pageInit: function(options){
			 ...
			 * @type {Obeject}
			 */
			opts = $.extend({},$.fn.zPager.defaults,options);
				...
				if(opts.url!=='' && typeof(opts.url)==='string'){
					...
					if(ajaxOpts.rows.length>0){
						var ishasDataRender = (opts.dataRender && typeof(opts.dataRender)==='function');
							ishasDataRender ? opts.dataRender(ajaxOpts.rows, opts, ajaxOpts.total) : methods.dataRender(_v, ajaxOpts.rows);
					}
				}else{
					$.pageError(2);
				}
